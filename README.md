...
- Web 検索対応（DuckDuckGo、API キー不要）— 必要に応じて検索し、引用付きで回答
...
Web 検索が行われた場合は、追加のイベントが流れます:

```
data: {"type": "tool", "tool": "web_search", "query": "検索クエリ"}

data: {"type": "sources", "sources": [{"title": "...", "url": "..."}]}
```

`done` イベントにはバックエンドが返す usage 情報と検索ソースが含まれます（対応している場合）:
